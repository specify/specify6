...
public class GeoCoordGeoLocateProvider implements GeoCoordServiceProviderIFace, Runnable {
	...
	public void processGeoRefData(List<GeoCoordDataIFace> items,
     ...
     // create a progress bar dialog to show the network progress
        ...
        //$NON-NLS-1$
        progressDialog.getCloseBtn().addActionListener(ae -> {
	...
	public void run() {
		for (GeoCoordDataIFace grItem: items)
        {
		 	if (!stopWork)
		 	{
				    ...
				    if (service != null)
				    {
    					...
    					if (port != null)
    					{
        					try 
        	                {
        						...
        						glResults.add(new Pair<>(grItem, glResultSet));
        					} 
        	                
        	                catch (RemoteException e) {
        						e.printStackTrace();
        						glResults.add(new Pair<>(grItem, new Georef_Result_Set()));
        					}
    					}
				    }
				...
				{
				    SwingUtilities.invokeLater(() -> UIRegistry.showError("Out of Memory! Please restart Specify."));
