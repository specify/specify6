 ...
 *
 */
public class EditorPropPanelOld extends JPanel
{
    private static final Logger  log                = Logger.getLogger(EditorPropPanelOld.class);
    
    protected ViewBasedDisplayPanel viewPanel    = null;
    protected FormViewObj           formViewObj  = null;
    protected DBTableInfo           tableInfo;
    
    protected FormCell              currentData = null;
    ...
    protected MultiView                          viewMultiView;
                           ...
                           final PropertyChangeListener        pcl)
    {
        ...
        if (addSaveBtn)
        {
            ...
            saveBtn.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e)
                {
                    ...
                    if (pcl != null)
                    {
                        pcl.propertyChange(new PropertyChangeEvent(EditorPropPanelOld.this, "accept", currentData, currentData)); //$NON-NLS-1$
                    }
                }
            });
        }
        ...
        viewDefMultiView.getCurrentView().getValidator().addEnableItem(saveBtn, FormValidator.EnableType.ValidAndChangedItems);
        
        
        viewMultiView = new MultiView(null,
                null, 
                AppContextMgr.getInstance().getView("SystemSetup", "ViewProps"),  //$NON-NLS-1$ //$NON-NLS-2$
                AltViewIFace.CreationMode.EDIT,
                MultiView.IS_EDITTING, null);

        viewMultiView.getCurrentView().getValidator().setEnabled(true);
        viewMultiView.getCurrentView().getValidator().addEnableItem(saveBtn, FormValidator.EnableType.ValidAndChangedItems);
     ...
     * @return
     */
    public MultiView getViewDefMultiView()
    {
        return viewDefMultiView;
    }

    /**
     * @return
     */
    public MultiView getViewMultiView()
    {
        return viewMultiView;
    }
     ...
     * @param formViewDef
     */
    public void setFormViewDef(FormViewDef formViewDef)
    {
     ...
     * @param viewDef
     */
    public void loadFormViewDef(final FormViewDef viewDef)
    {
        removeAll();
        
        add(viewDefMultiView, BorderLayout.CENTER);
        
        addSaveBtn(viewDefMultiView);
    ...
    public void loadView(final View view)
    {
        ...
        add(viewMultiView, BorderLayout.CENTER);
        
        addSaveBtn(viewMultiView);
        
        viewMultiView.setData(view);
        validate(); 
    }
    ...
    {
        removeAll();
        viewPanel = new ViewBasedDisplayPanel(null, "Editor", viewName, null, null, null, true, 0); //$NON-NLS-1$
        viewPanel.getMultiView().getCurrentView().getValidator().setEnabled(true);
        viewPanel.getMultiView().getCurrentView().getValidator().addEnableItem(saveBtn, FormValidator.EnableType.ValidAndChangedItems);
        if (viewPanel != null && viewPanel.getMultiView() != null)
        {
            add(viewPanel, BorderLayout.CENTER);
            
            addSaveBtn(viewPanel.getMultiView());
            
            validate();
            
            formViewObj = (FormViewObj)viewPanel.getMultiView().getCurrentView();
            tableInfo   = DBTableIdMgr.getInstance().getByClassName(className);
     ...
     * @param colInx
     */
    public void setDataIntoUI(final FormCell fc, 
                              final int      rows,
                              final int      rowDefs,
                              final int      rowInx,
                              final int      cols,
     ...
     * @param colInx
     */
    public void setDataIntoUI(final FormCellField fcf, 
                              final int           rows,
                              final int           rowDefs,
                              final int           rowInx,
                              final int           cols,
