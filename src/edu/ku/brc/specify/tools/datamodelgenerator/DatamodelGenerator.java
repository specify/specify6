...
public class DatamodelGenerator
{
 ...
 private Table createTable(Element aElement)
 {
  ...
  if(tableData==null)log.error("Could not retrieve TableMetaData from hashtable for table: " + tableName);
  String id = tableData.getId();
  String view = tableData.getDefaultView();
  return new Table(tableName, aElement.attributeValue("table"),
  ...
  try
  {
   ...
   if (dbNode != null)
   {
    for (Iterator i = dbNode.elementIterator("table"); i.hasNext();)
    {
     ...
     log.debug("Creating TableMetaData and putting in hashtable for name: " + tablename + " id: " + id + " defaultview: " +defaultView);
