...
public class ExtractorBusRules extends BaseBusRules {

    /**
     * @param dataClasses
     */
    public ExtractorBusRules() {
    ...
    public STATUS processBusinessRules(final Object parentDataObj, final Object dataObj, final boolean isExistingObject) {
        ...
        if (isExistingObject &&
                ...
                dataObj instanceof Extractor) {
            DNASequence ds = (DNASequence) parentDataObj;
            Extractor col = (Extractor) dataObj;

            Hashtable<Integer, Boolean> hash = new Hashtable<Integer, Boolean>();
            for (Extractor extractor : ds.getExtractors()) {
                Integer id = extractor.getAgent().getAgentId();
                boolean isBad = false;
                if (hash.get(id) == null) {
                    if (extractor.getId() != null && id.equals(col.getAgent().getAgentId())) {
                        isBad = true;
                    }
                    hash.put(id, true);
                } else {
                ...
                if (isBad) {
                    reasonList.add(String.format(getResourceString("DNASEQ_DUPLICATE_EXTRACTORS"), col.getIdentityTitle()));
                    return STATUS.Error;
                }
            }
        }
