...
/**
 * This classes is the abstract "base" class implementation of the model that is used
 * in the JTables. Classes are derived from this class so they can show more or less information
 * for the items that are being displayed.
 */
public class TableViewObjModel extends AbstractTableModel
{
  ...
  public void changeView(Node node)
  {
      ...
      try 
      {
          NodeList colList = XPathAPI.selectNodeList(aNode, "columns/column");
          for (int i=0;i<colList.getLength();i++) 
          {
              Node node = colList.item(i);
              String label = XMLHelper.findAttrValue(node, "label");
              
              // Get the Value for the TextField
              String fieldName = XMLHelper.findAttrValue(node, "name");
              ...
              if (!_isAttr)
              {
                  ...
                  {
                     typeClass = _class.getDeclaredField(fieldName).getType();
                  } else
                  {
              ...
              ColumnInfo colInfo = new ColumnInfo(label, typeClass, fieldName);
              _colInfo.addElement(colInfo);
  ...
  public Object getValueAt(int aRow, int aColumn)
  {
      ...
      Object obj = _data.get(aRow);
      ColumnInfo colInfo = (ColumnInfo)_colInfo.elementAt(aColumn);
      Object value = "";
      try
      {
          if (_isAttr)
          {
              PrepAttrs pa = (PrepAttrs)_data.get(aRow);
              if (colInfo._fieldName.equals("fieldname"))
              {
                  value = pa.getName();
              } else if (colInfo._fieldName.equals("value"))
              {
                  value = pa.getValue();
              } else
              {
          ...
          {
              return _dataGetter.getFieldValue(obj, colInfo._fieldName);
          }
      } catch (Exception e)
      {
          System.err.println(e+"\n"+colInfo._fieldName);
